...
public class MinHeap<T extends Comparable<T>> implements Heapish<T> {
    ...
    public void insert(T data) {
        ...
        while (pos > 0) {
            int parent = (pos - 1) / 2;
            if (data.compareTo(storage[parent]) >= 0) break;
    ...
    public T extract() {
        ...
        while (true) {
            ...
            int minChild = (rightChild < N && storage[rightChild].compareTo(storage[leftChild]) < 0)
                    ? rightChild : leftChild;
            if (data.compareTo(storage[minChild]) <= 0) break;
